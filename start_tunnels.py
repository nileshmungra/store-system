...
def find_free_port() -> int | None:
    ...
    for port in PREFERRED_PORTS:
        if is_port_free(port):
            ...
            logging.debug(f"  ✖ Port {port} is in use or blocked — trying next...")
    return None
...
def start_cloudflare_tunnel(port: int, use_ssl: bool):
    ...
    try:
        ...
        logging.warning("⚠️  cloudflared.exe not found in project folder.")
        return None
    ...
    for line in tunnel_process.stdout:
        stripped = line.strip()
        if stripped:
            logging.debug(f"  [cloudflare] {stripped}")
        ...
        if any(kw in stripped for kw in [
            ...
            logging.warning("⚠️  Cloudflare tunnel encountered an error — stopping tunnel attempt.")
        ...
        if time.time() > deadline:
            logging.warning("⚠️  Cloudflare tunnel timed out after 30 seconds.")
            tunnel_process.terminate()
            return None
...
def main():
    logging.debug("===================================================")
    logging.debug(" INVENTORY SYSTEM - QUICK TEMPORARY TUNNEL STARTER")
    logging.debug("===================================================")
    ...
    if port is None:
        ...
        logging.debug("   Please free one of these ports and try again.")
    ...
    scheme  = "https" if use_ssl else "http"
    if not use_ssl:
        logging.debug("ℹ️  SSL certificates not found — starting without SSL.")
    ...
    if tunnel_result and isinstance(tunnel_result, tuple):
        ...
        logging.debug("   (Accessible only on this Wi-Fi / LAN network)")

    # ── 6. Print the summary ──────────────────────────────────────────────────
    logging.debug("\n===================================================")
    logging.info(f" ✅ LIVE URL: {live_url}")
    logging.debug("===================================================")
    logging.debug(f" 📌 Dashboard: {live_url}/dashboard")
    logging.debug(f" 📌 Scanner:   {live_url}/scanner")
    logging.debug(f" 📌 Reports:   {live_url}/report-page")
    logging.debug(f" 📌 Log Book:  {live_url}/logs-page")
    logging.debug("===================================================")
    ...
    try:
        ...
        logging.warning(f"⚠️  File write error: {e}")
