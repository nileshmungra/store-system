...
# ===============================================
# 8. Search QRs API Tests (with mocked DB)
# ===============================================

def test_search_qrs_pagination_and_dates():
    with patch("main.get_db_ctx") as mock_get_db_ctx:
        mock_ctx, _, cursor = _mock_get_db_ctx()
        cursor.fetchone.return_value = {"total": 45}
        cursor.fetchall.return_value = [
            {
                "box_id": "BOX-001",
                "item_name": "PVC Fitting",
                "qty_in_box": 50,
                "status": "IN_STORE",
                "created_at": "2026-09-10 10:00:00",
                "batch_id": 1,
                "inward_date": "2026-09-10 10:00:00",
                "supplier_or_party": "ABC Corp",
                "remark": "Batch 1",
                "unit": "Pcs",
                "machine_name": None,
                "pipe_type": None,
                "pipe_size": None,
                "coil_length_meters": None,
                "coil_weight_kg": None,
                "shift_operator": None,
            }
        ]
        ...
        response = client.get("/api/search-qrs?supplier_only=true&limit=25&page=1&from_date=2026-09-01&to_date=2026-09-15")
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "Success"
        ...
        assert data["boxes"][0]["box_id"] == "BOX-001"
